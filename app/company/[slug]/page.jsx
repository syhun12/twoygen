...
const history = [
  { year: "2025", events: [["02", "인도 원전용 역세정(BW) 여과장치 2차분 공급(수출)"], ["03", "해외 프로젝트 대응 및 공급 역량 확대"], ["06", "액화수소 설비용 극저온 제어밸브 공급"], ["09", "차세대 에너지용 밸브 기술개발 고도화"]] },
  { year: "2024", events: [["05", "수소가스 생산·공급설비용 자동제어 밸브 개발"], ["08", "초저온 밸브 관련 등록 및 품질체계 확대"], ["10", "LNG 벙커링 설비용 밸브 공급"], ["12", "인도 원전용 역세정(BW) 여과장치 설계개발 및 1차분 공급(수출)"]] },
...
export default function CompanyPage({ params }) {
  ...
  if (slug === "history") {
    return <><SubHero eyebrow="회사소개" title="주요연혁" description="기술 혁신과 도전의 역사를 이어갑니다." /><section className="page-shell"><div className="container"><Tabs current={slug} /><div className="content-title"><span className="eyebrow">History</span><h2>Technical Innovation Timeline</h2><p>기술개발, 인증, 공급 프로젝트와 원자력 분야 수행 이력을 중심으로 주요 연혁을 정리했습니다.</p></div><div className="timeline">{history.map((item) => <div className="timeline-row" key={item.year}><div className="timeline-year">{item.year}</div><div className="timeline-events">{item.events.map(([month, text]) => <div className="timeline-event" key={month + text}><span>{month}</span><p>{text}</p></div>)}</div></div>)}</div></div></section></>;
  }
